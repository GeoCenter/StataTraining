...
* library
* [http://blog.rstudio.org/2015/04/09/readr-0-1-0/](readr::read_csv)
* head
* [http://yihui.name/knitr/](knitr::kable)
* summary
* [https://cran.rstudio.com/web/packages/dplyr/vignettes/introduction.html](dplyr::glimpse)
...
*Note: if you haven't installed the packages (groups of functions), you'll get an error if you try to load them with `library`.  To install `dplyr`, for instance, you should first run `install.packages('dplyr')`.*
...
# 'kable' is a function which formats info into a neat table which can be 
kable(head(spent))
```
